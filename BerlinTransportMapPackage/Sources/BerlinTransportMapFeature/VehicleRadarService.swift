...
actor VehicleRadarService {
    ...
    func fetchVehicles(
        ...
        components.queryItems = [
            ...
            URLQueryItem(name: "results", value: "500"), // More vehicles for wider area
    ...
    func fetchDepartures(stopId: String, duration: Int = 60) async throws -> [RESTDeparture] {
        // URL encode the stop ID properly to handle any special characters
        ...
        print("Fetching departures from URL: \(url.absoluteString)")
        ...
        print("Departures API response status: \(httpResponse.statusCode)")
        ...
        print("Decoded \(departuresResponse.departures.count) departures")
    ...
    func fetchTripRoute(tripId: String) async throws -> TripRoute? {
        ...
        print("Fetching trip route from URL: \(url.absoluteString)")
        
        let (data, response) = try await session.data(from: url)
        
        guard let httpResponse = response as? HTTPURLResponse,
              (200...299).contains(httpResponse.statusCode) else {
            print("Trip route API error")
            return nil
        }
        
        let decoder = JSONDecoder()
        let tripResponse = try decoder.decode(TripResponse.self, from: data)
        print("Trip route decoded successfully")
...
struct PolylineGeometry: Decodable {
    let type: String?
    let coordinates: [Double]? // Single [longitude, latitude] for Point type
}
...
struct Vehicle: Identifiable, Decodable {
    ...
    enum CodingKeys: String, CodingKey {
        case tripId, line, direction, location
    }
}
...
struct VehicleLine: Decodable {
    ...
    var color: String {
        // Use API-provided color if available, otherwise use default BVG colors
        if let bg = colorData?.bg, !bg.isEmpty {
            return bg
        }
        switch productType {
        case .suburbanTrain: return "#008C3C" // BVG S-Bahn green
        case .subway: return "#0066CC" // BVG U-Bahn blue
        case .tram: return "#D8232A" // BVG Tram red (official color)
        case .bus: return "#993399" // BVG Bus purple
        case .ferry: return "#0099CC" // BVG Ferry cyan
        case .regionalTrain: return "#EC192E" // DB Regional red
        }
    }
    
    var foregroundColor: String {
        // Use API-provided foreground color if available
