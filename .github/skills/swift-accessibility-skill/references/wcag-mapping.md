...
| 1.2.4 Captions (Live) | AA | Captions for live audio in video | `AVSpeechSynthesizer` for generated captions; third-party captioning services |
...
| 1.4.13 Content on Hover/Focus | AA | Dismissible, hoverable, persistent | SwiftUI: `.hoverEffect()`, `.focusable()`; ensure tooltips/popovers remain visible and dismissible |
...
| 2.4.7 Focus Visible | AA | Keyboard focus indicator visible | System provides default focus rings; do not remove them; customize with `.focusStyle()` |
...
| 2.5.7 Dragging Movements | AA | Single-pointer alternative to drag | Provide button-based reorder; `.accessibilityAction(.move(…))` |
...
- **Drag alternatives**: 2.5.7 requires single-pointer alternatives to drag. Provide button-based reordering or `.accessibilityAction(.move(…))`.
