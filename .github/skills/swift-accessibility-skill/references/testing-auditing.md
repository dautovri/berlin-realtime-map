...
- [AXFeatureOverrideSession (iOS 17+)](#axfeatureoverridesession-ios-17)
...
### AXFeatureOverrideSession (iOS 17+)

Enables specific accessibility features for test duration, resetting automatically when the session ends. Use in UI test targets.

```swift
import AXFeatureOverrideSession

class AccessibilityTests: XCTestCase {
    func testWithReduceMotion() throws {
        // Enable Reduce Motion for this test only
        let session = try AXFeatureOverrideSession(.reduceMotion, enabled: true)

        let app = XCUIApplication()
        app.launch()

        // Test that animations are suppressed
        let element = app.otherElements["AnimatedCard"]
        element.tap()

        // Verify state change happened without animation
        XCTAssert(app.otherElements["CardDetail"].exists)

        session.invalidate()  // or let it deinit
    }

    func testWithIncreasedContrast() throws {
        let session = try AXFeatureOverrideSession(.increaseContrast, enabled: true)
        // Test your UI with increased contrast active
    }
}
```
...
#### Combine with AXFeatureOverrideSession

Test audits under specific accessibility settings:

```swift
func testAuditWithDynamicType() throws {
    // Enable large text for the test
    let session = try AXFeatureOverrideSession(.largeText, enabled: true)

    let app = XCUIApplication()
    app.launch()

    // Audit for clipping and layout issues at large text sizes
...
Add to your test plan or scheme to run on every PR. A single test covers the most common accessibility regressions:
...
**Tip:** Run `performAccessibilityAudit()` after each navigation to audit every screen in your app. This catches issues that only appear on specific screens.
