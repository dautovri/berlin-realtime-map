...
## Linked Groups (iOS 17+)

Links non-adjacent elements that are semantically related, allowing VoiceOver to group them when using the Rotor.

```swift
@Namespace var photoGroup

// Image and its caption — displayed far apart in a grid
Image("sunset")
    .accessibilityLinkedGroup(id: "photo1", in: photoGroup)

// Caption may be in a different VStack
