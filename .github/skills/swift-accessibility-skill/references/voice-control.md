...
```swift
// Provide a way to pause or extend auto-hide
class PlayerControlsView: UIView {
    var autoHideTimer: Timer?
    var controlsAlwaysVisible = false

    // Reset timer on any assistive technology interaction
    override func accessibilityActivate() -> Bool {
        resetAutoHideTimer()
        return true
    }

    // Extend or disable auto-hide when any assistive tech is active
    // Note: there is no public API to detect Voice Control specifically.
    // Use broad assistive tech checks instead.
    func resetAutoHideTimer() {
        if UIAccessibility.isVoiceOverRunning
            || UIAccessibility.isSwitchControlRunning
            || controlsAlwaysVisible {
            return  // don't auto-hide when assistive tech is active
        }
        // normal timer logic
    }
}
